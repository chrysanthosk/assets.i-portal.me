...
<div class="d-inline-flex gap-1 flex-wrap justify-content-end">
    @if(! $p->isPaid())
        <form method="POST" action="{{ route('payments.markPaid', $p) }}" class="d-inline" data-no-loading>@csrf
            <button class="btn btn-sm btn-success" title="Received as expected" aria-label="Yes, received"><i class="bi bi-check2"></i><span class="d-none d-md-inline"> Yes</span></button>
        </form>
        @if($p->status === \App\Models\RentalPayment::STATUS_PENDING)
            <form method="POST" action="{{ route('payments.markNotReceived', $p) }}" class="d-inline" data-no-loading>@csrf
                <button class="btn btn-sm btn-outline-danger" title="Not received" aria-label="No, not received"><i class="bi bi-x"></i><span class="d-none d-md-inline"> No</span></button>
            </form>
        @endif
    @endif
    <button type="button" class="btn btn-sm btn-outline-secondary" title="Correct the amount" aria-label="Correct the amount"
            data-adjust
    ...
    <form method="POST" action="{{ route('payments.statement', $p) }}" enctype="multipart/form-data" class="d-inline" id="{{ $stmtId }}">@csrf
        ...
        <button type="button" class="btn btn-sm btn-outline-primary" title="Upload the manager's statement and take the payout from it" aria-label="Upload statement"
