    ...
    <div class="card">
      <div class="card-header d-flex align-items-center justify-content-between">
        <div>
          ...
          <small class="text-muted">Monthly rental income per asset</small>
        </div>
      </div>
      ...
      <div class="card-body">
        ...
        <h6 class="mb-2">Add / Update monthly income</h6>
        <div class="text-muted small mb-2">
          <strong>Year/Month</strong> = the month you received (or expect) income for that period (not the signed date).
        </div>
        ...
        <form method="POST" action="{{ route('assets.rentals.storeOrUpdate') }}" class="row g-2">
          ...
          <div class="col-md-2">
            ...
            <input type="number" name="month" class="form-control @error('month') is-invalid @enderror"
                   value="{{ old('month', now()->month) }}" min="1" max="12" required>
          ...
          <div class="col-md-4">
            ...
            <input type="text" name="tenant_name" class="form-control @error('tenant_name') is-invalid @enderror"
                   value="{{ old('tenant_name') }}" placeholder="e.g. John Smith / Airbnb guest">
          ...
          <div class="col-md-3">
            ...
            <input type="date" name="agreement_start_date" class="form-control @error('agreement_start_date') is-invalid @enderror"
                   value="{{ old('agreement_start_date') }}">
          ...
          <div class="col-md-3">
            ...
            <select name="rent_type" class="form-select @error('rent_type') is-invalid @enderror">
              <option value="">—</option>
              @foreach(['Airbnb','Long-term','Other'] as $t)
                <option value="{{ $t }}" @selected(old('rent_type') === $t)>{{ $t }}</option>
              @endforeach
            </select>
          ...
          <div class="col-md-4">
            ...
            <input name="channel" class="form-control @error('channel') is-invalid @enderror"
                   value="{{ old('channel') }}" placeholder="Airbnb / Long-term / Booking">
            ...
            <tbody>
              @forelse($rentals as $r)
                <tr>
                  ...
                  <td>{{ sprintf('%04d-%02d', $r->year, $r->month) }}</td>
                  <td>{{ $r->tenant_name ?: '—' }}</td>
                  <td>
                    @php
                      $start = $r->agreement_start_date ? \Illuminate\Support\Carbon::parse($r->agreement_start_date)->format('Y-m-d') : null;
                      $end   = $r->agreement_end_date ? \Illuminate\Support\Carbon::parse($r->agreement_end_date)->format('Y-m-d') : null;
                    @endphp
                    {{ $start ?: '—' }} → {{ $end ?: '—' }}
                  </td>
                  <td>{{ $r->rent_type ?: '—' }}</td>
                  ...
                  <td>{{ number_format((float)$r->amount, 2) }} {{ $r->currency }}</td>
                  <td>{{ $r->channel ?: '—' }}</td>
                  ...
                  <td class="text-end">
                    ...
                    @can('manage_asset_rentals')
                      <button
                        ...
                        data-delete-body="Delete rental record for <b>{{ e($r->asset?->name ?? 'Asset') }}</b> ({{ sprintf('%04d-%02d', $r->year, $r->month) }})?">
              ...
              @empty
                <tr>
                  <td colspan="10" class="text-center text-muted py-4">No rental records yet.</td>
                </tr>
              @endforelse
            </tbody>
          </table>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Delete Rental Record</h5>
