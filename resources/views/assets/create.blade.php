      ...
      <div class="card-body">
        <form method="POST" action="{{ route('assets.store') }}" class="row g-3">
          ...
          <div class="col-md-3">
            ...
            <select class="form-select @error('type') is-invalid @enderror" name="type" required>
              @foreach(['Apartment','House','Land','Commercial','Other'] as $t)
                <option value="{{ $t }}" @selected(old('type','Apartment') === $t)>{{ $t }}</option>
              @endforeach
            </select>
          ...
          <div class="col-md-3">
            ...
            <select class="form-select @error('status') is-invalid @enderror" name="status" required>
              @foreach(['Vacant','Owner-occupied','Rented (long-term)','Airbnb/Short-term'] as $s)
                <option value="{{ $s }}" @selected(old('status','Vacant') === $s)>{{ $s }}</option>
              @endforeach
            </select>
          ...
          <div class="col-md-4">
            ...
            <input class="form-control @error('owner_entity') is-invalid @enderror" name="owner_entity" value="{{ old('owner_entity') }}" placeholder="Personal / Company / Joint">
            @error('owner_entity') <div class="invalid-feedback">{{ $message }}</div> @enderror
          </div>

          <div class="col-md-3">
            <div class="form-check mt-4">
              <input class="form-check-input" type="checkbox" value="1" id="title_deed" name="title_deed" @checked(old('title_deed'))>
          ...
          <div class="col-md-3">
            ...
            <input class="form-control @error('title_deed_number') is-invalid @enderror" name="title_deed_number" value="{{ old('title_deed_number') }}">
            @error('title_deed_number') <div class="invalid-feedback">{{ $message }}</div> @enderror
          </div>

          <div class="col-md-3">
            <label class="form-label">Title Deed Date</label>
            <input type="date" class="form-control @error('title_deed_date') is-invalid @enderror" name="title_deed_date" value="{{ old('title_deed_date') }}">
